...
int main(void)
{
  ...
  System *sys = CreateSystem(dim, nx, ny, maxn, 0.01, 50.0, 1000, 7);
  double xl = 1.0;
  double yl = 1.0;


  /* double rho_min[maxn], v_min[dim][maxn], drhodt[maxn]; */
  /* double dvdt[dim][maxn], av[dim][maxn], xl =1.0, yl = 1.0 ; */
	...
	for(int i=0; i < sys->ntotal; i++)
	  {
	    ...
	    for(int d = 0; d < sys->dim; d++)
	      {
		sys->Velocity_min[i][d]  = sys->Velocity_min[i][d];
